  ...
  return (
    <div style={{ ...styles.container, padding: isMobile ? '20px 16px' : '34px 48px' }}>
      ...
      <div style={{ ...styles.createRow, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
        ...
        />
        <button onClick={handleCreate} style={{ ...styles.addBtn, flex: isMobile ? 1 : undefined }}>Adicionar</button>
        <button onClick={() => setBulkOpen(true)} style={{ ...styles.bulkBtn, flex: isMobile ? 1 : undefined }}>Colar vários</button>
      </div>
      ...
      {loading ? (
        ...
        <div style={styles.list}>
          {topics.map((t) => (
            <div key={t.id} style={{ ...styles.row, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
              <button
              ...
              {/* Grupo de ações — no mobile vai pra 2ª linha, alinhado à direita */}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  editInput: { flex: 1, minWidth: 0, padding: '7px 10px', borderRadius: 8, border: `1.5px solid ${theme.teal}`, background: theme.card, fontSize: 15, color: theme.ink, fontFamily: 'inherit', outline: 'none' },
