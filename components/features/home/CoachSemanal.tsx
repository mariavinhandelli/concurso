...
export function CoachSemanal() {
  ...
  if (!data || !data.hasData || dismissed) return null;
    ...
    <>
      <div style={s.banner}>
        ...
        <div style={s.bannerActions}>
          ...
          <button onClick={fechar} style={s.bannerDismiss} aria-label="Dispensar">✕</button>
        </div>
      </div>
      ...
      {modalOpen && (
        <div style={s.overlay} onClick={() => setModalOpen(false)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={s.actions}>
              <button onClick={fechar} style={s.fecharBtn}>Fechar</button>
              <button onClick={() => { fechar(); router.push('/performance'); }} style={s.verMaisBtn}>Ver desempenho completo →</button>
            </div>
          </div>
        </div>
      )}
