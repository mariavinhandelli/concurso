...
// Constância: barra horizontal de 60 dias, colunas finas de ponta a ponta.
// Verde-escuro = bateu meta · verde-claro = estudou · tijolo = falhou (quebrou
// sequência) · cinza = ainda não comecei. Frase + recorde em cima, legenda embaixo.
'use client';
...
export function StreakBar() {
  ...
  // Pega os últimos 60 dias (lastDays já vem ordenado do mais antigo ao recente).
  const dias = info.lastDays.slice(-60);

  // Descobre o primeiro dia com estudo: antes dele, vazio = "não comecei" (cinza);
  // depois dele, vazio = "falhou" (vermelho).
  ...
  return (
    <div style={styles.wrap}>
      ...
      {/* Barra de 60 colunas finas, de ponta a ponta */}
...
const styles: Record<string, React.CSSProperties> = {
  wrap: { fontFamily: theme.font, width: '100%' },
  ...
  bar: { display: 'flex', gap: 3 },
  cell: { flex: 1, height: 26, borderRadius: 3, transition: 'opacity .1s' },
