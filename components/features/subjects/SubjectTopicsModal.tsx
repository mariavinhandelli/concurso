...
export function SubjectTopicsModal({ subject, topics, loading, activating, onActivate, onClose }: Props) {
  ...
  return (
    <Overlay
      onClose={() => { if (!activating) onClose(); }}
      closeOnBackdrop={!activating}
      labelledBy="subject-topics-title"
      maxWidth={600}
      padding={0}
    >
      <div style={{ maxHeight: 'min(82vh, calc(100dvh - 80px))', display: 'flex', flexDirection: 'column' }}>
        {/* Cabeçalho */}
        <div style={{
          padding: '20px 24px 16px',
          borderBottom: `0.5px solid ${theme.line}`,
        }}>
          <h2 id="subject-topics-title" style={{ margin: 0, fontSize: 18, fontWeight: 700, color: theme.ink, letterSpacing: -0.4 }}>
            {subject.name}
          </h2>
